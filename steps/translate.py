...
def translate(chain):
    ...
    if last_step["type"] == 'get_neuroscience_concepts_and_possible_translations':
        ...
        if chain.current_step > 1 and chain.steps[chain.current_step - 2]["type"] == "critique":
            critiques = chain.steps[chain.current_step -
                                    2]["output"] + "\n\n" + last_step["output"]
        else:
        ...
        if response.choices and response.choices[0].message.content:
            print("Follow up translation after critiques: " +
                  response.choices[0].message.content)
            return response.choices[0].message.content
    elif last_step["type"] == 'find_and_summarize_relevant_papers':
        two_steps_ago = chain.steps[chain.current_step - 2]
        prompt = base_prompt + \
            "Take this as context for the translation: " + two_steps_ago["output"] + \
            "And here are summarized papers that may help: " + last_step["output"] + \
            "Okay, now let's translate the following:" + \
            chain.input
        ...
        if response.choices and response.choices[0].message.content:
            print("Initial translation with relavent papers as context: " +
                  response.choices[0].message.content)
            return response.choices[0].message.content
